...
- urls.py에서 클라이언트의 요청에 따라 적절한 view로 연결시켜줌
...
- 템플릿을 띄워주는 위해 django.shortcuts 모듈의 render 함수를 사용하는데, 이 함수의 첫 번째 인자 또한 request
...
- url 설정에 정의된 특정한 경로들의 의존성을 제거할 수 있음
  - 앱 내 [views.py](http://views.py) 파일: `path('index/', views.index)` → `path('index/', views.index, name='index'),`
